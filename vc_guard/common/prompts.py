...
例如，假设现在已观测象限列表为 [2]，那么表示右上象限已被观测过，就不要再派遣车辆前往右上象限了。

2. 终点坐标选定规范
需在任务地点坐标附近选定新终点，且新坐标需与任务地点处于对应象限的对角线上，坐标偏移规则固定如下：
- 左上象限（索引 1）：(13, 13)；
- 右上象限（索引 2）：(13, 17)；
- 左下象限（索引 0）：(17, 13)；
- 右下象限（索引 3）：(17, 17)。
优先选择已观测象限列表里面没有的象限索引，并给出对应的新坐标位置。**在所有象限没有观测完毕之前不要出现象限重复**
特殊情况：若所有象限（0-3）均已观测，则随机选择任一象限，按上述规则选定终点后派遣车辆。
你只能在上述几个坐标里选，不要自己选定了。
...
5. **额外注意**：你是在已有的车辆里，派遣车辆前往新终点，你无需关注车辆的象限位置，只需要关注新终点的象限和已观测象限尽量不重合。
