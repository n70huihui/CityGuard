...
你的任务是根据当前的地图信息（0 表示障碍，1 表示通路，其余数字表示道路拥堵程度，数字越大拥堵程度越高），然后派遣车辆前往。
地图会以拍摄任务地点为中心，分割成为四个象限：左下、左上、右上、右下，分别对应象限索引为 0、1、2、3。
观测过的象限应该尽可能避开，避免重复观测。
如果你要选择车辆，你应该现在地图上，根据任务地点的坐标附近，选择一个新的终点（尽量避开障碍和拥堵路段），然后派遣这辆车前往。
例如：假设现在观测过的象限为 [0, 1]，那么你应该在任务地点坐标附近的右下或者右上位置选一个终点，然后派遣车辆前往。
为了保证象限选择的正确，请保证你选择的新坐标和任务地点坐标(i, j)保持在同一对角线上。（左上：(i - x, j - x), 右上: (i - x, j + x), 左下: (i + x, j - x), 右下: (i + x, j + x)）
例如：假设任务地点坐标为 (12, 13)，然后你的新坐标要选在左上方，那么可以的新坐标为：(11, 12), (10, 11) ...
如果发现观测过的象限为 [0, 1, 2, 3]，此时所有象限全部被观测过，那么你就随机选择一个象限，选定终点后派遣即可。
你要返回的是，你选择的车辆 id 列表以及对应的终点坐标。坐标的表示和二维数组的表示一致，都是先行后列。
## 任务信息
当前地图信息: {grid_matrix}
当前观测过的象限信息: {observed_quadrant}
...
6. 返回最终的车辆 id 列表和终点坐标列表，数量需要和需要选择的车辆一致，如果当前车辆数量总数少于需要选择的车辆数量，则返回当前所有车辆 id 列表。如果没有可用车辆，直接返回空列表即可。
""")
